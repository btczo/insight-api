...
class TransactionDb {
  ...
  async _fillOutpoints (txInfo) {
    ...
    await Promise.map(txInfo.vin, async (i) => {
      const ret = this.fromTxIdN(i.txid, i.vout);
  ...
  async fromAddr (addr, opts = {}) {
    return new Promise(async (resolve, reject) => {
      ...
      .on('end', async () => {
        await Promise.map(ret.filter(x => !x.spentIsConfirmed), async (o) => {
  ...
  async fillScriptPubKey (txOuts) {
    await Promise.map(txOuts, async (txOut) => {
  ...
  async _addManyFromHashes (txs) {
    let dbScript = [];
    await Promise.map(txs, async (tx) => {
