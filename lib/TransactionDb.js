...
const genesisTXID = 'c083fb7c3b6936c15dc2685a522ffa685247e8c665c818888b51b0771584d7b4';
...
class TransactionDb {
  ...
  async _fillOutpoints (txInfo) {
    ...
    await Promise.each(txInfo.vin, async (i) => {
      ...
      i.value = but.Unit.fromSatoshis(ret.valueSat).toBUT();
      valueIn += i.valueSat;
    ...
    if (!incompleteInputs) {
      txInfo.valueIn = but.Unit.fromSatoshis(valueIn).toBUT();
      txInfo.fees = but.Unit.fromSatoshis(valueIn - but.Unit.fromBUT(txInfo.valueOut).toSatoshis()).toBUT();
    } else {
  ...
  _fromBuffer (buf) {
    const buf2 = but.util.buffer.reverse(buf);
  ...
  getStandardizedTx (tx, time, isCoinBase) {
    ...
    tx.vout = tx.outputs.map((txout) => {
      ...
      let valueSat;
      if (txout.s) {
        const s = new but.Script(txout.s);
        const addrs = new but.Address.fromScriptPubKey(s, config.network);
        ...
        const s = new but.Script(txout.script);
  ...
  _addScript (tx, relatedAddrs) {
    ...
    for (let ii in tx.vout) {
      ...
      if (o.scriptPubKey && o.scriptPubKey.addresses && o.scriptPubKey.addresses[0] && !o.scriptPubKey.addresses[1]) {
        ...
        const sat = o.valueSat || but.Unit.fromBUT(o.value).toSatoshis();
