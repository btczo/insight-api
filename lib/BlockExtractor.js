...
const Parser = digibyte.encoding.BufferReader;
...
class BlockExtractor {
  constructor(dataDir, network) {
    ...
    this.network = network === 'testnet' ? digibyte.Networks.testnet: digibyte.Networks.livenet;
  ...
  async getNextBlock () {
    ...
    const b = digibyte.Block.fromBufferReader(this.currentParser);
    if(this.blocks.length <= BLOCK_DOWNLOAD_WINDOW) {
      this.blocks.push(b.toObject());
    }
    var nextBlock = this.blocks.filter((o) => {
      return o.header.prevHash === this.prevHash;
    })[0];
    if(!nextBlock) {
    ...
    return new digibyte.Block(nextBlock);
  }
}
