...
class BlockDb {
  ...
  async getBlockForTx (txId) {
    try {
      ...
      const v = val.split(':');
      return { hash: v[0], height: parseInt(v[1]) };
    } catch (e) {
      return { hash: null, height: null };
    }
  }
  ...
  async _fillConfirmationsOneSpent (o, chainHeight) {
    ...
    if (o.multipleSpentAttempts) {
      await Promise.each(o.multipleSpentAttempts, async (oi) => {
  ...
  async _fillConfirmationsOne (o, chainHeight) {
    const { hash, height } = await this.getBlockForTx(o.txid);
    if (height >= 0) {
  ...
  async fillConfirmations (txOuts) {
    ...
    await Promise.each(txs, async (txOut) => {
